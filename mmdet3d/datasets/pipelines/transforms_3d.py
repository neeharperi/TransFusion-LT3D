...
class ObjectSample(object):
    ...
    def __init__(self, db_sampler, start_point_cloud_range=None, end_point_cloud_range=None, sample_2d=False):
...
@PIPELINES.register_module()
class ObjectNameExpansion(object):
    """Allow each object to be classified by a non-exclusive heirarchy
    Args:
        classes: Standard ontology prior to expansion
        task_names: class_names for each of the parts of the hierarchy
        class_mapping: mapping between class_names and task_names
    """
    ...
    def process_labels(self, gt_bboxes_3d, gt_labels_3d):
        ret_bboxes, ret_labels = [], []
        box_dim = gt_bboxes_3d.box_dim
        bboxes = gt_bboxes_3d.tensor.numpy()

        total_classes = []
        for task in self.task_names.keys():
            total_classes += self.task_names[task] 

        name_mapping = {}
        for task in self.task_names:
            for class_name, mapping in zip(self.task_names[task], self.class_mapping[task]):
                for name in mapping:
                    if name not in name_mapping:
                        name_mapping[name] = []

                    name_mapping[name].append(class_name)
    
        for i, label in enumerate(gt_labels_3d):
            box = bboxes[i]
            name = self.classes[label]

            for alt_name in name_mapping[name]:
                ret_bboxes.append(box)
                ret_labels.append(total_classes.index(alt_name))
        
        ret_bboxes = np.stack(ret_bboxes)
        ret_bboxes  = LiDARInstance3DBoxes(ret_bboxes, box_dim=box_dim)
        ret_labels = np.array(ret_labels)

        return ret_bboxes, ret_labels

    def __call__(self, input_dict):
        """Call function to apply process_labels to each ground truth in the scene.
        Args:
            input_dict (dict): Result dict from loading pipeline.
        Returns:
            dict: Results after name expansion to each object,
                'gt_labels_3d', 'gt_bboxes_3d' keys are updated in the result dict.
        """
        gt_bboxes_3d = input_dict['gt_bboxes_3d']
        gt_labels_3d = input_dict['gt_labels_3d']

        gt_bboxes_3d, gt_labels_3d = self.process_labels(gt_bboxes_3d, gt_labels_3d)
    ...
    def __repr__(self):
        ...
        repr_str += f' task_names={self.task_names},'
        repr_str += f' class_mapping={self.class_mapping},'
        return repr_str
...
@PIPELINES.register_module()
class ObjectRangeFilterInterval(object):
    """Filter objects by the range.
    Args:
        point_cloud_range (list[float]): Point cloud range.
    """

    def __init__(self, start_point_cloud_range, end_point_cloud_range):
...
@PIPELINES.register_module()
class PointsRangeFilterInterval(object):
    """Filter points by the range.
    Args:
        point_cloud_range (list[float]): Point cloud range.
    """
