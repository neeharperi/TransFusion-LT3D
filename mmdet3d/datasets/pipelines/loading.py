...
@PIPELINES.register_module()
class LoadPointsFromFileFeather(LoadPointsFromFile):
    def _load_points(self, pts_filename, reference_SE3_ego, timestamp_delta):
        """Private function to load point clouds data.
        Args:
            pts_filename (str): Filename of point clouds data.
        Returns:
        ...
        sweep = read_feather(pts_filename, columns=["x", "y", "z", "intensity", "laser_number"])
        coordinates_m = (sweep.loc[:, ["x", "y", "z"]].to_numpy().astype(np.float32))
        intensity = sweep.loc[:, "intensity"].to_numpy()
        laser_number = sweep.loc[:, "laser_number"].to_numpy()

        coordinates_m = reference_SE3_ego.transform_point_cloud(coordinates_m)
        points = np.zeros((len(coordinates_m), 6))
        points[..., :3] = coordinates_m
        points[..., 3] = intensity * 1.0 / 255
        points[..., 4] = timestamp_delta * 1e-9
        points[..., 5] = laser_number       
    ...
    def __call__(self, results):
        ...
        try:
            if "sweeps" in results:
                pts_filename = results['sweeps'][0]
                reference_SE3_ego = results['transforms'][0]
                timestamp_delta = results['timestamp_deltas'][0]
                points = self._load_points(pts_filename, reference_SE3_ego, timestamp_delta)
            else:
                pts_bytes = self.file_client.get(results["pts_filename"])
                points = np.frombuffer(pts_bytes, dtype=np.float32)
...
@PIPELINES.register_module()
class LoadPointsFromMultiSweepsFeather(LoadPointsFromMultiSweeps):
    _load_points = LoadPointsFromFileFeather._load_points
    ...
    def __call__(self, results):
        ...
        for i in range(len(results['sweeps'])):
            pts_filename = results['sweeps'][i]
            reference_SE3_ego = results['transforms'][i]
            timestamp_delta = results['timestamp_deltas'][i]
            points = self._load_points(pts_filename, reference_SE3_ego, timestamp_delta)

            aggregate_points.append(points)

        points = np.concatenate(aggregate_points, axis=0)
