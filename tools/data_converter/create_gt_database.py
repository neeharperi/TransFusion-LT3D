...
def create_groundtruth_database(dataset_class_name,
    ...
    elif dataset_class_name == 'AV2Dataset':	
        dataset_cfg.update(	
            pipeline=[	
            dict(	
    ...
    if database_save_path is None:
        database_save_path = osp.join(data_path, f'{out_path}/{info_prefix}_gt_database')
    if db_info_save_path is None:
        db_info_save_path = osp.join(data_path, f'{out_path}/{info_prefix}_dbinfos_train.pkl')
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        for i in range(num_obj):
            ...
            rel_filepath = osp.join(f'{out_path}/{info_prefix}_gt_database', filename)
